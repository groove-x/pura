# Toy HTTP server for Trio, derived from example server of python-hyper/h11.
#
# Changes (which upstream is not interested in adopting):
#  * http_serve() takes request_handler and debug options
#  * factor out receive_body handler
#  * serve() propagates kwargs to http_serve()
#  * lint fixes
#
# MIT License
# Copyright (c) 2016 Nathaniel J. Smith <njs@pobox.com> and other contributors
#
# https://github.com/python-hyper/h11/blob/master/examples/trio-server.py
...
async def maybe_send_error_response(wrapper, exc):
    ...
    try:
        ...
        wrapper.info("error while sending error response:", exc2)
